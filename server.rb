    ...
    if message.chat
      ...
      when Telegram::Bot::Types::Message
        ...
        when /Украина/
          ...
          bot.api.send_message(chat_id: message.chat.id, text: answers.sample)
        when /ч/
          answers = ["#{message&.chat&.first_name} иди пососи хуйца", 'Нам похуй', '- уважение к тебе', 'подрочить хочешь?', 'Ты мне блять хуйню не заливай', 'Ты нищий какие нюдсы', 'Денег дай, а потом проси', 'Мне было грустно и одиноко и я увидела его, банан', 'Где суши сука влад блять', 'Бля, пойду повешусь', 'О выеби нас', 'хавайся']
          bot.api.send_message(chat_id: message.chat.id, text: answers.sample)
        when /мать/
          answers = ["#{message&.chat&.first_name} иди пососи хуйца", 'Где гроши сука влад блять', 'Хантер х хантер, лучшее аниме', 'А как по другому мне играть в твою гейскую кс', 'подрочить хочешь?', 'Ты мне блять хуйню не заливай','Ты нищий какие нюдсы', 'Денег дай, а потом проси', 'Мне было грустно и одиноко и я увидела его, банан', 'Где суши сука влад блять', 'Бля, пойду повешусь', 'О выеби нас', 'хавайся']
          bot.api.send_message(chat_id: message.chat.id, text: answers.sample)
        when /нюдсы/
        ...
        when /или/
          ...
          bot.api.send_message(chat_id: message.chat.id, text: answers.sample)
        when /dick/
          answers = ['Иди нахуй', 'Получается Илья что ты гей', '- уважение к тебе', 'Дима вас слушает', 'хватит оргазмировать', 'Сука Артем', '-10 см в члене', 'Где гроши сука влад блять', 'Это примерно, хз я же не лизун']
          bot.api.send_message(chat_id: message.chat.id, text: answers.sample)
        when /Top/
          answers = ['Иди нахуй', 'Получается Илья что ты гей', '- уважение к тебе', 'Дима вас слушает', 'хватит оргазмировать', 'Сука Артем', '-10 см в члене', 'Где гроши сука влад блять', 'Это примерно, хз я же не лизун']
          bot.api.send_message(chat_id: message.chat.id, text: answers.sample)
        ...
        when /енот/
          ...
          bot.api.send_message(chat_id: message.chat.id, text: answers.sample)
        when /он/
          answers = ["чпокнул #{message&.chat&.first_name}", 'Где гроши сука влад блять', 'Закрой поддувало', 'Нам похуй', 'Ослепла от моего члена?', 'Ебаный бот', 'енот нормальный', 'Шутки шутками, но в попку то больно', 'геев не люблю', 'Жук тётя шлюха', 'не хочу в попку', 'енот пидр']
          bot.api.send_message(chat_id: message.chat.id, text: answers.sample)
        when /гей/
